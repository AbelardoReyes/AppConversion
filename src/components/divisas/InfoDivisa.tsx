...
export default function InfoDivisa() {
  ...
  const handleDeleteDivisa = (Symbol: string) => {
    const allDivisas = JSON.parse(localStorage.getItem("allDivisas") || "[]");
    const index = allDivisas.findIndex(
      (divisa: Divisa) => divisa.symbol === Symbol,
    );
    allDivisas.splice(index, 1);
    localStorage.setItem("allDivisas", JSON.stringify(allDivisas));
    router(`/divisas`);
  };
        ...
        <>
          {editInput ? (
            <div className="flex w-full items-center justify-center gap-4">
              <button
                onClick={() => handleDeleteDivisa(form.symbol)}
                className={`mt-2 w-1/2 rounded-xl bg-red-500 p-2 text-white hover:bg-red-600`}
              >
                <p className="w-full text-center text-white">Eliminar</p>
              </button>
            </div>
          ) : (
            <div className="flex flex-row items-center justify-between gap-4">
              ...
              >
                <p className="w-full text-center text-white">Guardar</p>
              </button>
            </div>
          )}
        </>
